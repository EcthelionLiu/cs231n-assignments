...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # Query (N, S, E) and Key (N, T, E)
        Q = Q.reshape(N, S, H, d_h).transpose(1, 2)
        K = K.reshape(N, T, H, d_h).transpose(1, 2)
        V = V.reshape(N, T, H, d_h).transpose(1, 2)
        
        # Query (N, H, S, d_h) and Key (N, H, T, d_h)
        score = torch.matmul(Q, K.transpose(2, 3)) / math.sqrt(d_h)
        ...
        # contiguous()让数据在内存空间中连续，然后才能用view转换类型
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None):
        ...
        # cross attention
        res_connect1 = tgt
        cross_atten = self.cross_attn(query = tgt, key = memory, value = memory)
        cross_atten = self.dropout_cross(cross_atten)
        cross_atten = cross_atten + res_connect1
        cross_atten = self.norm_cross(cross_atten)
        
        # feed forward
        res_connect2 = cross_atten
        out = self.ffn(cross_atten)
        out = self.dropout_ffn(out)
        out = out + res_connect2
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        d_H, d_W = H // self.patch_size, W // self.patch_size
        P = self.patch_size
        x = x.reshape(N, C, d_H, P, d_W, P)
        x = x.permute(0, 2, 4, 1, 3, 5).contiguous()
        x = x.reshape(N, d_H * d_W, C * P * P)
