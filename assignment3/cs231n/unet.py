...
class Unet(nn.Module):
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        cond_kwargs.setdefault("text_emb", None) 
 
        uncond_kwargs = copy.deepcopy(model_kwargs)
        uncond_kwargs["text_emb"] = None

        eps_cond = self.forward(x, time, cond_kwargs)      # ε̂(x_t,t,c)
        eps_uncond = self.forward(x, time, uncond_kwargs)  # ε̂(x_t,t,φ)
        
        x = (1.0 + cfg_scale) * eps_cond - cfg_scale * eps_uncond
        ##################################################################
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        ##################################################################
        downs_info = []
        for down_blocks in self.downs:
            downs_info.append([x := b(x, context) for b in down_blocks[:-1]])
            x = down_blocks[-1](x)
        
        x = self.mid_block2(self.mid_block1(x, context), context)
        
        
        for up_blocks, res_info in zip(self.ups, reversed(downs_info)):
            x = up_blocks[0](x)
            layer_num = len(up_blocks) - 1
            
            for i in range(0, layer_num):
                x = torch.cat([x, res_info[layer_num - 1 - i]], dim = 1)
                x = up_blocks[i + 1](x, context)  
        ##################################################################
