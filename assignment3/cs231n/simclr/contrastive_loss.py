...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        up = torch.exp(sim(z_k, z_k_N)/ tau) 
        
        down1, down2 = torch.tensor(0.0), torch.tensor(0.0)
        for i in range(2 * N):
            if i == k: continue
        ...
        for j in range(2 * N):
            if j == k + N: continue
            down2 += torch.exp(sim(z_k_N, out[j])/ tau)
            
        loss = -(up / down1).log() - (up / down2).log()
        total_loss += loss
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
...
def sim_positive_pairs(out_left, out_right):
    ...
    # HINT: torch.linalg.norm might be helpful.                                  #
    ##############################################################################
    nl = torch.linalg.norm(out_left, dim = 1, keepdims = True)
    nr = torch.linalg.norm(out_right, dim = 1, keepdims = True)
    
    diag = torch.diag(out_left @ out_right.T).unsqueeze(1)
    
    pos_pairs = diag / (nl * nr)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return pos_pairs
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    denom = torch.sum(exponential, dim = 1).to(device)

    # Step 2: Compute similarity between positive pairs.
    # You can do this in two ways: 
    # Option 1: Extract the corresponding indices from sim_matrix. 
    # Option 2: Use sim_positive_pairs().
    sim_pair = torch.cat((torch.diag(sim_matrix[:N, N:]), torch.diag(sim_matrix[N:, :N])))
    
    # Step 3: Compute the numerator value for all augmented samples.
    numerator = torch.exp(sim_pair / tau).to(device)
    
    # Step 4: Now that you have the numerator and denominator for all augmented samples, compute the total loss.
    loss = - torch.mean((numerator / denom).log())
