...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        ############################################################################
        # serve as decoder input in this transformer decoder architectrue
        x = self.embedding(captions)
        x = self.positional_encoding(x)
        # (N, T, E)
        
        # picture input as encoder(but here we didn't have encoder)
        memory = self.visual_projection(features) # (N, E)
        memory = memory.unsqueeze(1) # (N, 1, E)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.positional_encoding(x)
        x = self.transformer(x) # (N, num_patch, embed_dim)
        
        x = torch.mean(x, axis = 1) # (N, embed_dim
