...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        initial_h = features @ W_proj + b_proj

        initial_word = word_embedding_forward(captions_in, W_embed)
        
        whole_h = rnn_forward(initial_word, initial_h, Wx, Wh, b)
        
        logits = temporal_affine_forward(whole_h, W_vocab, b_vocab)

        loss = temporal_softmax_loss(logits, captions_out, mask)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def sample(self, features, max_length=30):
        ...
        first_word = torch.full((N,), self._start, dtype=torch.long)
        
        for i in range(max_length):
          out = word_embedding_forward(first_word, W_embed)
          
          h = rnn_step_forward(out, h, Wx, Wh, b)
          
          scores = h @ W_vocab + b_vocab
          next_word = scores.argmax(axis = 1)
