...
# cs231n/im2col_cython.py  (pure NumPy fallback)
import numpy as np


def im2col_cython(x: np.ndarray, field_height: int, field_width: int,
                  padding: int, stride: int) -> np.ndarray:
    """
    x: (N, C, H, W)
    return: cols of shape (C * field_height * field_width, N * HH * WW)
    """
    N, C, H, W = x.shape
    ...
    for yy in range(HH):
        ...
        for xx in range(WW):
            w0 = xx * stride
            # 窗口： (N, C, field_height, field_width)
            window = x_padded[:, :, h0:h0 + field_height, w0:w0 + field_width]
            # 重排为 (C*fh*fw, N)
            cols[:, out_col * N:(out_col + 1) * N] = window.transpose(1, 2, 3, 0).reshape(-1, N)
            out_col += 1

    return cols


def col2im_cython(cols: np.ndarray, N: int, C: int, H: int, W: int,
                  field_height: int, field_width: int,
                  padding: int, stride: int) -> np.ndarray:
    """
    cols: (C * field_height * field_width, N * HH * WW)
    return: (N, C, H, W)
    """
    HH = (H + 2 * padding - field_height) // stride + 1
    WW = (W + 2 * padding - field_width) // stride + 1
    ...
    for yy in range(HH):
        ...
        for xx in range(WW):
            w0 = xx * stride
            # 取出 (C*fh*fw, N) -> (C, fh, fw, N) -> (N, C, fh, fw)
            block = cols[:, in_col * N:(in_col + 1) * N].reshape(
                C, field_height, field_width, N
            ).transpose(3, 0, 1, 2)
            x_padded[:, :, h0:h0 + field_height, w0:w0 + field_width] += block
            in_col += 1
...
def col2im_6d_cython(cols: np.ndarray, N: int, C: int, H: int, W: int,
                     HH: int, WW: int, pad: int, stride: int) -> np.ndarray:
    """
    6D 版本（有些作业代码会用到）：
    cols: (C, HH, WW, N, out_h, out_w)
    HH/WW 在此语义上等价于 field_height/field_width
    return: (N, C, H, W)
    """
    out_h = (H + 2 * pad - HH) // stride + 1
    out_w = (W + 2 * pad - WW) // stride + 1
    assert cols.shape == (C, HH, WW, N, out_h, out_w), \
        f"cols shape should be (C, HH, WW, N, {out_h}, {out_w}), got {cols.shape}"
    ...
    for n in range(N):
        for c in range(C):
            for hh in range(HH):
                h_slice = slice(hh, hh + stride * out_h, stride)
                for ww in range(WW):
                    w_slice = slice(ww, ww + stride * out_w, stride)
                    # 把 (out_h, out_w) 累加到对应位置
                    x_padded[n, c, h_slice, w_slice] += cols[c, hh, ww, n]
